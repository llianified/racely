...
export function RaceBattery({
  ...
  const status =
    battery.phase === "discharging"
      ? `Boost ${game.economy.boostMultiplier}× aktif`
      : battery.phase === "charging"
        ? `Siap dalam ${battery.readyIn} detik`
        : "Siap untuk Gaspol";

  return (
    <section
      className="race-battery"
      data-phase={battery.phase}
      aria-label="Baterai cadangan boost"
    >
      <button
        type="button"
      ...
      >
        <span className="battery-power">
          <strong className="battery-value" aria-hidden="true">
            {battery.percent}
            <small>%</small>
          </strong>
          <span
            ...
            role="progressbar"
            aria-label="Daya baterai boost"
            aria-valuemin={0}
            aria-valuemax={100}
            aria-valuenow={battery.percent}
            aria-valuetext={`${battery.percent} persen. ${status}`}
          >
                ...
                <span
                  style={{
                    transform: `scaleX(${Math.max(
                      0,
                      Math.min(1, battery.charge * 10 - index),
                    )})`,
                  }}
                />
              </span>
