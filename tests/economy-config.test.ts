...
const NON_COIN_FIELDS = [
  ...
  "rivalTrackingStrength", "rivalDailyJitter",
...
describe("Proyeksi ekonomi", () => {
  it("menerjemahkan config bawaan jadi rupiah per jam", () => {
    ...
    // Sejak lawan mengikuti level pemain, pemain level 1 finis P2, bukan P3
    // permanen: 0,05 koin tiap 8 detik = 22,5 koin/jam = Rp2.250. Dulu 18
    // koin/jam, saat lawan statis selalu mengalahkan pemain baru.
    expect(rows[0].coinsPerHour).toBeCloseTo(22.5);
    expect(rows[0].idrPerHour).toBeCloseTo(2250);
    // Kenaikan ini tidak menembus pagar: atap harian tetap yang mengikat.
    expect(rows[0].idrPerDayIdle).toBeLessThanOrEqual(
